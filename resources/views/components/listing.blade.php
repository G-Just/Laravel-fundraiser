...
<div class="relative">
    <a href="@if ($editable) {{ route('cause.edit', $id) }} @else {{ route('cause.show', $id) }} @endif"
        ...
        @if ($editable)
            <div
                class="absolute left-0 right-0 flex flex-col items-center justify-center -translate-y-1/2 bg-black bg-opacity-75 top-20 lg:top-1/2">
                <p>This cause is not yet approved</p>
                <p>Click to edit</p>
            </div>
        @endif
        <img class="object-cover w-screen rounded-t-lg max-h-64 lg:max-h-full lg:rounded-r-none lg:rounded-l-lg lg:w-1/3"
